// at the top of your file
const Discord = require('discord.js');

module.exports = {
	name: 'about',
	description: 'Max Players',
	async execute(message, args) {
		// url
		let alrpprofile = new URL('https://asialaneroleplayofficial.web.app');
		let site = new URL('https://asialaneroleplay.web.official');
		//embed
		const exampleEmbed = new Discord.MessageEmbed()
		.setColor('#00FDEE')
		.setAuthor('Asia Lane', 'https://cdn.discordapp.com/attachments/1211308635536490538/1211308893318283304/emoji-removebg-preview.png?ex=65f6f4d1&is=65e47fd1&hm=b0eac966739195229eb1d02a8f94077f24993485a230e889f948cd3ab3c18bb9&', 'https://discord.js.org')
		.setDescription('Tentang ALRP Bot \n Jika Ada Problem Atau Masalah Silahkan PM Discord Saya Atau Tag Saya Di Chat Admin')
		.addFields(
			{ name: 'Bot Version:', value: 'v4.0' , inline: true },
			{ name: 'Server Name:', value: 'Asia Lane Roleplay' , inline: true },
			{ name: 'Bot Status:', value: 'Online' , inline: true },
			{ name: '\u200B', value: '\u200B' },
			{ name: 'Author:', value: '[Aan Permadi]('+alrpprofile+'/)' , inline: true },
			{ name: 'Site:', value: '[Web Asia Lane]('+site+'/)' , inline: true },
		)
		.setTimestamp()
		.setFooter('Asia Lane Development Team', 'https://cdn.discordapp.com/attachments/1211308635536490538/1211308893318283304/emoji-removebg-preview.png?ex=65f6f4d1&is=65e47fd1&hm=b0eac966739195229eb1d02a8f94077f24993485a230e889f948cd3ab3c18bb9&');

		//send msg
		message.channel.send(exampleEmbed);
	},
};